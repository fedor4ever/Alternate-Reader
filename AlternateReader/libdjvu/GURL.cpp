...
#if defined(UNIX) || defined(OS2) || defined(__SYMBIAN32__)
# include <unistd.h>
# include <sys/types.h>
# include <sys/stat.h>
# include <errno.h>
...
GURL::is_local_file_url(void) const
{
   if(!validurl) const_cast<GURL *>(this)->init();
   GCriticalSectionLock lock((GCriticalSection *) &class_lock);
   bool returndata=(protocol()=="file" && url[5]==slash);
   return returndata;
}
...
//TODO:: SYMBIAN CHEKC
#if defined(UNIX) || defined(macintosh) || defined(OS2) || defined (__SYMBIAN32__)
static int
...
GURL::expand_name(const GUTF8String &xfname, const char *from)
{
  ...
  // UNIX implementation
//TODO: ÎÁßÇÀÒÅËÜÍÎ ÁËßÒÜ ÏÐÎÂÅÐÈÒÜ ÄËß ÑÈÌÁÛ ÝÒÓ ÕÐÅÍÜ !!
#if defined(UNIX) 
